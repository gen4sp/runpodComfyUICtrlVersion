...
-   **Docker + handler**: контейнер с хендлером для запуска воркфлоу под конкретную версию. Возврат результата как **base64 (по умолчанию)** или **ссылка на артефакт в Google Cloud Storage**.
...
-   `instructions.md` — руководство по использованию и параметрам (обновлено: Docker/handler, локальный запуск).
